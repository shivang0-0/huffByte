...
bool huffbytecompare0(huffbyte a, huffbyte b)
{
...
{
    std::string command = "echo \"huffByte\" | figlet | lolcat";
    ...
    // this array of type 'huffbyte' will not be used after calculating transformed versions of every unique byte
    // instead its info will be written in a new string array called str_arr
    huffbyte array[letter_count * 2 - 1];
    huffbyte *e = array;
    for (long int *i = number; i < number + 256; i++)
    {
        if (*i)
    ...
    sort(array, array + letter_count, huffbytecompare0);
    //---------------------------------------------
    ...
    huffbyte *min1 = array, *min2 = array + 1, *current = array + letter_count, *notleaf = array + letter_count, *isleaf = array + 2;
